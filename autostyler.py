# Autostyler.py
# Turns plain text, with NO MARKUP, into styled Word documents.
# This version (c) Craig Duncan 2022

# It needs an output folder called 'output' in the current directory.
#
# run as python3 autostyler.py filename.txt
# In input text file (whether called filename.txt or otherwise):
# include a line with meta:legaldoc or meta:letter or meta:memo to describe it.

import sml2docx as docmaker
import review
import xmlutil  #ooxmlutil as 
...
# StylesTemplateA uses H2 as a bold heading for h2code, and Indent in lvlnames
...
#
# lvl is used to record outline lvl state, and this influences styles
# 
	...
	for item in myArray:
		...
		if (first5=="meta:"):
			if ("letter" in item):
				...
				chooseLegalDocScheme() # this could also pass on a subscheme
				processed=True
			elif ("memo" in item):
				chooseMemoScheme()
				processed=True
...
# This processing will increase levels if needed
...
if (args<2):
	...
	print("i.e. python3 autostyler.py filename.txt")

elif (args==2 and __name__ == '__main__'):
	filename=sys.argv[1]
	if len(filename)!=0:
		# remainder is optional for now hey:
		# This assumes no multi-period filenames are used (cf file.knit.md)

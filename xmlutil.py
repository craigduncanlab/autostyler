...
# python3 version of zipfile: how to access/force?

import zipfile # the prefix for all functions.  Part of std python library.
import datetime
...
def utftest(filename,inputdata):
    testxml=filename.find("xml",0,len(filename))
    print("My xml test result for finding xml is:")
    print(testxml)
    testrels=filename.find("rel",0,len(filename))
    # if we find the chars 'xml' in the file...
    if testxml!=-1 or testrels!=-1:
        datanew = inputdata.decode("utf-8")
    ...
    with zipfile.ZipFile(newname, 'w',zipfile.ZIP_DEFLATED) as zipwrite:
        ...
        for item in nbpackages:
            if item.filename not in excludednames:  # no duplication
                ...
                if isUTF(item.filename,insertdata)==True:
                     zipwrite.writestr(item, newdata.encode('utf-8'))  
                else:
